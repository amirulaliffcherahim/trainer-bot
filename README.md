...
### Strava direct sync (optional)

Connect once, then `/sync` imports your runs with authoritative numbers
(no screenshots):

1. Create an app: https://www.strava.com/settings/api → note Client ID/Secret
2. `python strava_auth.py <client_id> <client_secret>` → follow the prompts
3. Add `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, `STRAVA_REFRESH_TOKEN` to `.env`
4. Restart, then `/sync` in Telegram

The bot refreshes the access token automatically and persists the rotated
refresh token. Only runs are imported, deduped by Strava id, verified, and
fed to predictions. Screenshots remain as a fallback for anything else.
